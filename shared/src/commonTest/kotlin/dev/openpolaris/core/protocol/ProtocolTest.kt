...
class ResponseParserTest {
    ...
    // -------- Regression: pre-fix findResponseStart() infinite-looped on request-only input --------

    @Test
    fun `request frame with no at-sign does not hang parser`() {
        // Bug history: ResponseParser.findResponseStart() used to reset i = atIdx + 1 = 0
        // whenever the buffer contained no '@', so any request frame (`1&<code>&2&...#`)
        // would loop forever and starve the per-client read thread.
        val wire = "1&524&2&#"
        val (frames, _) = parser.parse(wire.toByteArray())
        assertEquals(1, frames.size)
        assertEquals(524, frames[0].code)
        assertTrue(frames[0].fields.isEmpty())
    }

    @Test
    fun `partial buffer with no at-sign and no terminator returns no frames`() {
        // Even if the buffer is incomplete (no '#' yet), the parser must not loop.
        val wire = "1&284&2&mode:".toByteArray()
        val (frames, consumed) = parser.parse(wire)
