...
## [Unreleased]
### Added
- **Branded desktop application icon.** The Windows / macOS / Linux native
  distributions now ship a custom OpenPolaris icon (compass + Polaris motif)
  instead of the jpackage default: `openpolaris.ico` (Windows),
  `openpolaris.icns` (macOS) and `openpolaris.png` (Linux) are wired into
  `nativeDistributions`, the Inno Setup desktop/Start-Menu shortcuts point at
  the bundled `.ico`, and the Compose window sets a matching taskbar / title-bar
